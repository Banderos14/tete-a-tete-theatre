...
describe('релевантность даты спектакля', () => {
  ...
  it('ранняя проверка — справка о сеансе, а НЕ «билет на другую дату»', () => {
    // «Сегодня не 17 сентября» не означает «билет на другую дату»: так
    // корректный билет получал ложное предупреждение при каждом раннем скане.
    expect(page).toContain('isEarlyShow');
    expect(page).toContain('Спектакль состоится');
    // Ищем именно выводимый текст (шаблонную строку), а не упоминание
    // прежней формулировки в комментарии.
    expect(page).not.toMatch(/`Билет на другую дату/);
  });

  it('предупреждение о другой дате зависит только от showDateDiffers', () => {
    // Расхождение — это «бронь на один вечер, каталог на другой».
    // Момент сканирования (showRelevance) в это условие входить не должен.
    const warning = page.match(/const dateWarning = ([^;]+);/s);
    expect(warning, 'строка предупреждения о дате не найдена').not.toBeNull();
    expect(warning![1]).toContain('showDateDiffers');
    expect(warning![1]).not.toContain('isEarlyShow');
  });
});
