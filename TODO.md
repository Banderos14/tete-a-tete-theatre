...
-+- Deep-link для афиши работает: ссылка вида `/#/?show=<showId>` открывает сайт, прокручивает к репертуару и открывает модалку спектакля с кнопкой "Купить билет"
    - `/#/?show=romantika`
    - `/#/?show=shutka`
    - `/#/?show=nulin`
...
-+- Остаток мест приходит с сервера (/api/show-availability); если он неизвестен — индикатор скрыт
