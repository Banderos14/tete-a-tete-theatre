// Программа лояльности: посетите 4 спектакля — 5-й билет со скидкой 50 %.
//
...
//   • каждые LOYALTY_VISITS_PER_REWARD (4) посещения дают одну скидку 50 % на
//     ОДИН билет следующей брони;
...
// Единственное место, где задан порог: сервер (цена) и кабинет/форма брони
// (показ) берут его отсюда, тексты подставляют это же число.
export const LOYALTY_VISITS_PER_REWARD = 4;
...
export interface LoyaltySummary {
  ...
  visits:    number;
  /** Скидки, заработанные всего: floor(visits / LOYALTY_VISITS_PER_REWARD). */
  earned:    number;
  /** Скидки, занятые действующими бронями. */
  used:      number;
  /** Можно применить скидку к следующей брони. */
  available: boolean;
  /** Точки текущего цикла: 0…LOYALTY_VISITS_PER_REWARD; максимум — скидка готова. */
  progress:  number;
