...
6. **Проверь**: в письме нет ключей и токенов. Реквизиты — боевые, из `src/config/payment.ts`: сверь IBAN и BIC с выпиской Crédit Agricole.
...
3. Набери броней до предела `THEATRE_CAPACITY` (значение в `shared/catalog/shows.ts`).
...
- Реквизиты театра (IBAN/BIC/RIB) живут только в `src/config/payment.ts` — один источник правды, расхождение ловит `tests/unit/payment.test.ts`.
