...
**Телефон — одна реализация:** `shared/domain/phone.ts` (libphonenumber-js/min). Ввод любой привычный — «07…», «+33…», «0033…», «+380…»; номер без кода страны и без ведущего 0 не угадывается. Хранится E.164 (`+33749661940`), в полях показывается с пробелами; нераспознанная старая запись не переписывается. Форма брони и кабинет проверяют номер при уходе из поля и отправке, сервер (`validateCreateBooking`) нормализует ещё раз. Библиотека — только в ленивых чанках. Дата рождения в профиле необязательна: бронь, билеты и лояльность от неё не зависят.
